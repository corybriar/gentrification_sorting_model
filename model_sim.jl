...
function household_sort(para, space, firms, prices, rents, wages, γ, n, εh, εw,nloc)
    ...
    for m in 1:M
        ...
        for e in E
            for s in 1:S
                ...
                πlpgl = exp.(VW) .* ((ones(1,nloc[m])*(exp.(VW))').^(-1))
                # fill Vsm with utilities for each location under sector s
                if e == 1
                    Vsm[1:IU,:,s] = transpose((1/σh).*(diag(πlpgl .* transpose(VW)) .-
                        α[e].*log.(rents[m]) .- (1-α[e]).*log.(Pl[m]))) .+ εh[m][1:IU,:]
                else
                    Vsm[(IU+1):(IS + IU),:,s] = transpose((1/σh).*(diag(πlpgl .* transpose(VW)) .-
    ...
    Mmax = Int.(Mmax)
    Vmax = [maxm[1,2:3,Mmax]; Mmax']'

end

"""
MATRICES
firms: [id, city, location, sector]
prices: [price]
wages: w[m][ℓ,e,s]
